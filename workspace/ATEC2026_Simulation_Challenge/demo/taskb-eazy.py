...
class _Stage1BaseRuntime:
    ...
    def _candidate_policy_paths(self) -> list[str]:
        ...
        return [
            os.path.join(demo_dir, "taskb_stage1_9400.pt"),
    ...
    def _fixed_arm_action(self, proprio: torch.Tensor) -> torch.Tensor:
        idx = 12
        joint_pos = proprio[:, idx : idx + self.OFFICIAL_ACTION_DIM]
        current_arm = joint_pos[:, 16:24]
        target = self.ARM_HOME_REL.to(device=proprio.device, dtype=proprio.dtype).view(1, -1)
        target = target.repeat(proprio.shape[0], 1)
        return torch.clamp((target - current_arm) / self.ARM_SCALE, -1.0, 1.0)
